...
class CodeEditor(QPlainTextEdit):
    ...
    def smart_paste(self):
        """Paste with smart indentation adjustment"""
        ...
        # Reset cursor to insertion point
        cursor = self.textCursor()
        ...
        # Multi-line paste - adjust indentation
        # Find minimum indentation in pasted text (ignoring empty lines)
        ...
        # Adjust all lines relative to current indentation
        adjusted_lines = []
        for i, line in enumerate(lines):
            if not line.strip():  # Empty line
                adjusted_lines.append('')
            else:
                line_indent = self.get_line_indentation(line)
                relative_indent = line_indent - min_indent
                new_indent = current_indent + relative_indent

                # Remove old indentation and add new
                stripped_line = line.lstrip()
                new_line = self.create_indent_string(new_indent) + stripped_line
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        # Handle paste with smart indentation
        ...
        if event.key() in (Qt.Key_Return, Qt.Key_Enter):
            ...
            if cursor.hasSelection():
                ...
                if self.indent_type == "tabs":
                    ...
                    spaces_to_remove = min(self.indent_size, len(line) - len(line.lstrip(' ')))
    ...
    def mousePressEvent(self, event):
        ...
        # Always call parent for normal behavior
    ...
    def focusInEvent(self, event):
        """Reset cursor state when editor gains focus"""
        self.ctrl_pressed = False
        self.viewport().setCursor(Qt.IBeamCursor)
...
class FindDialog(QDialog):
    ...
    def hideEvent(self, event):
        """When dialog is hidden, reset editor's ctrl state"""
        super().hideEvent(event)
        if self.editor:
            self.editor.ctrl_pressed = False
...
class PythonEditor(QMainWindow):
    ...
    def show_find_dialog(self):
        ...
        # Reset ctrl state when opening dialog
        self.editor.ctrl_pressed = False
        self.editor.viewport().setCursor(Qt.IBeamCursor)
